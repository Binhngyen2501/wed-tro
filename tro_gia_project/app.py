...
class Room(Base):
    ...
    price_suggestions: Mapped[list[PriceSuggestion]] = relationship(back_populates="room")
    ...
    st.markdown(
        ...
        * {font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif !important;}
        ...
        .card-shell {
            ...
            border-radius: 20px;
            padding: 1.5rem;
            box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.02), 0 10px 20px -5px rgba(0, 0, 0, 0.04);
...
def sidebar_menu(user: SessionUser) -> str:
    with st.sidebar:
        ...
        st.markdown(
            """
            <div style="padding: 1rem 0 1.5rem; text-align: center; border-bottom: 1px solid rgba(255,255,255,0.1); margin-bottom: 1rem;">
                <h2 style="color: white; margin: 0; font-size: 1.5rem; font-weight: 700;">Tro Gia</h2>
                <p style="color: #94a3b8; margin: 0.25rem 0 0; font-size: 0.8rem;">Hệ thống quản lý phòng trọ</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    if unpaid:
        ...
        for p in unpaid:
            with st.container(border=True):
                ...
                with c2:
                    pay_key = f"show_qr_{p.payment_id}"
                    ...
                    if not st.session_state.get(pay_key, False):
                        ...
                        if st.session_state.get(confirm_key, False):
                            with get_db() as db:
                                p_updated = mark_payment_paid(db, p.payment_id, "momo_qr")
                                write_audit_log(
